...
code += "if(" + " ".join(condition) + ") {" + words[1] + "()}\n"
  ...
  code += (
    'document.getElementById("body").innerHTML += "' + lines[i].split('"')[1] + '<br>";\n'
  )
else :
  code += (
    'document.getElementById("body").innerHTML += ' + words[1] + '+"<br>";\n'
  )
...
code += " ".join(words) + ");\n"
  ...
  code += (
    "setTimeout(function() {" + words[1] + '(); document.getElementsByTagName("title")[0].innerHTML = title; document.getElementById("icon").href = icon}, ' + words[2] + ");\n"
  )
elif words[0] == "listen":
  if words[1] == "Space":
  code += (
    'document.addEventListener("keydown",(e)=>{if(e.key==="' + " " + '"){' + words[2] + "()}});\n"
  )
else :
  code += (
    'document.addEventListener("keydown",(e)=>{if(e.key==="' + words[1] + '"){' + words[2] + "()}});\n"
  )
...
with open("temp/" + sys.argv[1].replace(".room", "") + "/index.html", "wt") as f:
  f.write(
    "<head>\n  <title></title>\n  <link rel='icon' id='icon' />\n</head>\n<body>\n  <div id=\"body\"></div>\n  <script defer>\ntry{\n" + pre + code + post + "} catch (inernalError182939227374) {alert(inernalError182939227374); console.log(inernalError182939227374)}\n  </script>\n</body>"
  )
f.close()
